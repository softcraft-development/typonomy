...
import type { Explicit, IndexMapper, Optional, Possible, Predicate, Reducer, Some, Transform, TypeGuard } from "./types"
 ...
 */
export function forArray<T>(array: T[], callback: IndexMapper<T, void>): void {
 ...
 */
export function forSome<T>(some: Some<T>, callback: (value: Optional<T>, index: number) => void): void {
 ...
 */
export function mapArray<T, R>(array: T[], mapper: IndexMapper<T, R>): R[] {
 ...
 */
export function mapReducer<T, R>(mapper: IndexMapper<T, R>): Reducer<R[], Optional<T>, number> {
 ...
 */
export function mapSome<T, R>(some: Some<T>, mapper: IndexMapper<T, R>): Some<R> {
  if (isPlural(some)) {
...
export function reduceArray<S, T>(
  ...
  reducer: Reducer<S, Optional<T>, number>,
  initialState: S,
  startIndex = 0,
 ...
 * @returns The final state.
 */
export function reduceSome<S, V>(some: Some<V>, reducer: Reducer<S, Optional<V>, number>, initialState: S): S {
  if (isPlural(some)) {
...
export function typeGuardSome<T>(guard: TypeGuard<T>): TypeGuard<Some<T>> {
  const predicate = partialRight<unknown, TypeGuard<T>, boolean>(isSome, guard)
